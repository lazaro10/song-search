...
@Suite struct PlayerViewModelTests {
    ...
    @Test func startSetsErrorWhenPreviewURLIsMissing() async {
        ...
        let sut = PlayerViewModel(song: song, audioPlayer: player)
    ...
    @Test func startLoadsAndPlaysThenObservesUpdates() async {
        ...
        // Give the start task a chance to load + play + begin observing.
    ...
    @Test func startPausesOnTaskCancellation() async {
        let url = URL(string: "https://example.com/preview.m4a")!
        let player = AudioPlayerSpy()
        let sut = PlayerViewModel(song: SongFixture.make(previewURL: url), audioPlayer: player)
    ...
    @Test func skipForwardCallsPlayerWithPositiveInterval() {
        let (sut, player) = makeSUT(skipInterval: 15)
        sut.skipForward()
    ...
    private func makeSUT(skipInterval: TimeInterval = 10) -> (sut: PlayerViewModel, player: AudioPlayerSpy) {
        let player = AudioPlayerSpy()
